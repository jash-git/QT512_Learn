...
function a(e){
e=e||document.body.offsetHeight,y.invoke("configMpAdAttrs",{
viewHeight:e
},function(t){
...
var _=M.getCrtExpInfo(p),m=110===_.exp_type,f=g.isAndroid&&g.gtVersion("7.0.15",!0),c=p.pos_type===F.POS_MID&&"1"===M.getExpParaVal(p,100280),u=!!A.getQuery("forcewebcompt"),l=p.pos_type===F.POS_MID,h=p.pos_type===F.POS_AD_BEFORE_VIDEO,v=p.pos_type===F.POS_BOTTOM&&"1"===M.getExpParaVal(p,100316),w=p.pos_type===F.POS_BOTTOM;
if(l&&t++,c&&i++,w&&o++,f&&(s&&"error"===s.status&&M.report128729(64),s&&"loading"===s.status&&M.report128729(65)),
s&&"ready"===s.status&&f&&(c||v||m||u))r.push(p);else if(l||h&&0===p.is_mp_video||w||p.pos_type===F.POS_AD_AFTER_VIDEO||p.pos_type===F.POS_AD_MID_VIDEO||h&&1===p.is_mp_video||p.pos_type===F.POS_SPONSOR)n.push(p);else{
a.push(p);
var y=JSON.stringify({
adInfo:p,
dataType:"unrecognized_ad_info"
});
(new Image).src="/mp/jsmonitor?idkey=115849_72_1&lc=1&log0="+y+"&r="+Math.random();
}
}
if(o>1){
for(var I=[],O={
...
e.height&&(t.style.height=e.height),G&&a(parseInt(e.height,10)),this.aInfoMap[e.aid].aInfo.pos_type===F.POS_BOTTOM&&!this.hasReportBottomTime&&"5"===window.item_show_type){
...
try{
"on"===o.methodType?y[o.methodType](o.methodName,i):y[o.methodType](o.methodName,o.args,i);
}catch(n){
